...
/// Return 1 if a Queen can be placed at currentRow-th row and
...
/// labs(r) returns the absolute value of r.
{
    ...
    {
        if((columnOf[row] == currentColumn) /// Two in the same column
           || (labs(columnOf[row] - currentColumn) == labs(row - currentRow)))
...
int main()
{
    ...
    while(tolower(key) == 'y')
    {
        printf("\n\n\n\tFor n * n sized chess board, Enter number of Queens, n : ");
        scanf("%llu", &nSize);
