...
int NextValue(unsigned long long currentVertex)
{
    ...
    /// have been assigned integer values in
...
int mColoring(unsigned long long currentVertex)
{
    ...
    /// All assignments of 1,2,...,maxColor of to the
    /// vertices of the graph such that adjacent vertices are
    /// assigned distinct integers are printed. currentVertex is the index of the
    /// next vertex to the color.
    ...
    while(true)
    {
        ...
        /// Assign to colorOf[currentVertex] to a legal color
...
int main()
{
    ...
    while(tolower(key) == 'y')
    {
        ...
        printf("\n\n\tAll possible color combinations of %llu vertices are : ",
               totalVertices);
        maxColor = 2;
        while(mColoring(0) == 0)
        {
