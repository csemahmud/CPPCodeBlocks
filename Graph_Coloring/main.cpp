...
int NextValue(unsigned long long currentVertex)
{
	...
	while (true)
	{
		...
		for (vertex = 0; vertex < totalVertices; vertex++)
		{
			...
			{
				/// If ( currentVertex, vertex ) is an edge and if adj.
...
int mColoring(unsigned long long currentVertex)
{
	...
	while (true)
	{
		...
		/// Assign to colorOf[currentVertex] a legal color
...
int main()
{
	...
	while (tolower(key) == 'y')
	{
		...
		printf("\n\n\tAll possible color combinations of %llu vertices are : ", totalVertices);
