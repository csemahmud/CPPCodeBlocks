...
/// Find all Subsets of elements[ 0 : range-1 ] that sum to givenSum. The values of flag[j]
/// 0<=j<index, have already been determined
/// currentSum = SUM_OF( elements[j]*flags[j], where 0<=j<index )
/// and remainingSum = SUM_OF( elements[j], where index<=j<range ). The elements[j] are
...
int main()
{
    ...
    while(tolower(key) == 'y')
    {
        ...
        /// elements[10] = 30.5;
        /// givenSum = 30.5
        /// for(unsigned long long = 0; i < range; i++)
        /// {
        ///     totalSum += elements[i];
        /// }
        ...
        if(range < 100000000)
        {
            ...
            printf("\n\tThe set of the given elements are : \n\t{\t%lf",
                   elements[0]);
            for(i = 1; i < range; i++)
            {
                printf(",\t%lf", elements[i]);
            ...
            {
                printf("\n\tERROR : givenSum : %lf must be greater than elements[0] : %lf",
                       givenSum, elements[0]);
            }
            ...
            {
                printf("\n\tAll possible combinations of Sum OfSubsets are : ");
